...
function array2xml($array, $wrap='result', $upper=false) {
...
$app->get('/irctcservices/stationcode/:id', 'authenticate', function($task_id) {
            ...
            if ($result != NULL) {
				...
				$response["totaltrains"] = $result["totaltrains"];     
				
                echoXML(200, $response);
            } else {
                $response["error"] = true;
                $response["message"] = "The requested stationcode doesn't exists";
                echoXML(404, $response);
            }
        });
		
		
$app->get('/services', 'listservices');
//$app->get('/services/stationcode/:query', 'getStationCode');

function listservices(){
$response = array("/ticketdiary/stationcode/:id", "/ticketdiary/trainstatus/:id");
 echoXML(404, $response);
}

function getStationCode($query){
